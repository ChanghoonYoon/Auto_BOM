...
for merged_range in ws.merged_cells.ranges:
    if merged_range.min_col == 2:  # B열(카테고리 열)
        ws.cell(row=merged_range.min_row, column=2).alignment = Alignment(horizontal='center', vertical='center')
...
for column in ws.columns:
    ...
    ws.column_dimensions['A'].width = 6  # A열의 너비를 6으로 설정
    ws.column_dimensions['F'].width = 12  # F열의 너비를 12으로 설정
    ws.column_dimensions['E'].width = 40  # G열의 너비를 16으로 설정
    ws.column_dimensions['J'].width = 16  # J열의 너비를 16으로 설정
...
# 파일 저장
excel_file = 'BOM_merge_update_v2.3_clunm_width.xlsx'
wb.save(excel_file)
